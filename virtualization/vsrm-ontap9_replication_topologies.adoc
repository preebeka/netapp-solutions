...
When you use array-based replication (ABR) in SRM, protection groups are isolated to a single array pair, as shown in the following screenshot. In this scenario, `SVM-A1` and `SVM-A2` are peered with `SVM-B1` and `SVM-B2` respectively at the recovery site. However, you can select only one of the two array pairs when you create a protection group.
